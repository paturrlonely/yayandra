...
async function pagi(hanz, x, data) {
    ...
    return await hanz.sendMessage(x.room, {
        text: pagii(x),
        contextInfo: {
            externalAdReply: {
...
async function voting(hanz, x, data) {
    let row = [];
    let ment = [];
...
async function malam(hanz, x, data) {
...
async function skill(hanz, x, data) {
    skillOn(x.room, data)
        ...
        for (let i = 0; i < x.player.length; i++) {
            if (x.player[i].role === "werewolf") {
                if (x.player[i].isdead != true) {
                    ...
                    await hanz.sendMessage(x.player[i].id, {
                        text: textnya,
                        mentions: membernya,
                    });
                }
...
async function win(x, t, hanz, data) {
    ...
    if (getWinner(x.room, data).status === false || x.iswin === false) {
        ...
        return await hanz.sendMessage(sesinya, {
            text: textnya,
            contextInfo: {
                externalAdReply: {
                    title: "W E R E W O L F",
...
async function run(hanz, id, data) {
    while (getWinner(id, data).status === null) {
        ...
        if (getWinner(id, data).status != null) {
            ...
            await pagi(hanz, sesi(id, data), data);
        }
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, hanz, data);
            break;
        ...
        if (getWinner(id, data).status != null) {
            ...
            await voting(hanz, sesi(id, data), data);
        }
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, hanz, data);
            break;
        ...
        if (getWinner(id, data).status != null) {
            ...
            await malam(hanz, sesi(id, data), data);
        }
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, hanz, data);
            break;
        } else {
            await skill(hanz, sesi(id, data), data);
        }
        if (getWinner(id, data).status != null) break;
    }
    await win(sesi(id, data), 1, hanz, data);
}

async function run_vote(hanz, id, data) {
    while (getWinner(id, data).status === null) {
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, hanz, data);
            break;
...
async function run_malam(hanz, id, data) {
    while (getWinner(id, data).status === null) {
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, hanz, data);
            break;
...
async function run_pagi(hanz, id, data) {
    while (getWinner(id, data).status === null) {
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, hanz, data);
            break;
