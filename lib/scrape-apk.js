...
let apkcombo = {
    search: async function(args) {
        ...
        for (let i = 0; i < Math.max(name.length, link.length); i++) {
            ress.push({
                name: name[i],
                link: link[i]
            });
...
let apkdl = {
    ...
    download: async function(url) {
        ...
        let link1 = $('head > meta:nth-child(11)').attr('content').replace('0; url=', '');
...
let aptoide = {
    search: async function(args) {
        let res = await fetch(tools.api(5, '/apps/search', { query: args, limit: 1000 }));
        res = await res.json();
    ...
    download: async function(id) {
        let res = await fetch(tools.api(5, '/apps/search', { query: id, limit: 1 }));
        res = await res.json();
