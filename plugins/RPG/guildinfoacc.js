let handler = async (m, { hanz, args }) => {
    ...
    if (guild.owner !== userId) return hanz.reply(m.chat, 'Hanya pemilik guild yang bisa melihat informasi guild lain.', m);

    let targetUser = global.db.data.users[target];
    if (!targetUser || !targetUser.guild || targetUser.guild !== guildId) return hanz.reply(m.chat, 'User tidak tergabung dalam guild kamu.', m);
...
Dibuat Pada: ${guild.createdAt}`;
    ...
    hanz.reply(m.chat, guildInfo, m, { mentions: [guild.owner, ...guild.members] });
};
