let handler = async (m, { command, usedPrefix,hanz,DevMode, args }) => {
      ...
      switch (type) {
        ...
        case "diamond":
          if (user.rawdiamond >= count * 6 && user.coal >= count) {
            ...
            user.coal -= count;
            user.diamond += count;
            hanz.reply(m.chat, `Sukses melebur ${count} diamond`, m);
          } else {
            hanz.reply(
              m.chat,
        ...
        case "iron":
          if (user.rawiron >= count * 2 && user.coal >= count) {
            ...
            user.coal -= count;
            user.iron += count;
            hanz.reply(m.chat, `Sukses Melebur ${count} Besi `, m);
          } else {
            hanz.reply(
              m.chat,
