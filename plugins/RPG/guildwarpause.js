let handler = async (m, { hanz }) => {
    ...
    if (guild.owner !== userId) return hanz.reply(m.chat, 'Hanya pemilik guild yang bisa menghentikan perang.', m);

    // Logika untuk menghentikan perang dapat ditambahkan di sini

    hanz.reply(m.chat, 'Pertempuran dengan guild lawan sedang dihentikan.', m);
};
