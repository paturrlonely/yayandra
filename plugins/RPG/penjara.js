let handler = async (m, { hanz, args, text, usedPrefix, command }) => {
 ...
 if (usar.job == 'polisi') {
    ...
    setTimeout(() => {
    hanz.reply(who, `*Kamu telah di penjara oleh ${usar.name}*`, fverif)
    }, 5000)
    hanz.reply(m.chat, `Berhasil penjara *@${(who || '').replace(/@s\.whatsapp\.net/g, '')}*\n🧤 +1 Tingkat Kerja Keras\n\n_Jika polisi diketahui memenjarai seseorang tanpa alasan tertentu, maka akan langsung diban oleh pihak atasan._`, m, { mentions: [who] })
    return
   }
   await hanz.reply(m.chat, '*Fitur ini hanya dikhususkan untuk orang yang bekerja sebagai polisi*', m)
}
