...
let handler = async (m, { hanz, args, participants, usedPrefix, command }) => {
  ...
  let imgr = 'https://telegra.ph/file/e0a337aec3a7901d8dfb2.png'
      ...
      m,
      {
        contextInfo: {
          externalAdReply: {
            ...
            mediaType: 1,
            title: "",
            thumbnailUrl: await (await fetch(imgr + "leaderboard")).buffer(),
            renderLargerThumbnail: true,
            mediaUrl: 'https://telegra.ph/file/89b52f3200ebb5cc04322.jpg',
            sourceId: wm,
            sourceUrl: web,
          },
        },
    ...
    (user, i) =>
      `${i + 1}.*﹙${user[type]}﹚*- ${
        participants.some((p) => areJidsSameUser(user.jid, p.id))
          ? `${user.registered ? user.name : hanz.getName(user.jid)} \nwa.me/`
          : "ғʀᴏᴍ ᴏᴛʜᴇʀ ɢʀᴏᴜᴩ\n @"
      }${user.jid.split`@`[0]}`
  ).join`\n\n`}
`.trim();
  return await hanz.reply(m.chat, text, m, {
    contextInfo: {
