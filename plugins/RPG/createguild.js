...
let handler = async (m, { hanz, args }) => {
    if (args.length < 1) return hanz.reply(m.chat, 'Contoh penggunaan: .createguild <nama_guild>', m);
    ...
    if (!user) return hanz.reply(m.chat, 'Kamu belum terdaftar di dalam database.', m);
    
    if (user.money < 20000000000) {
        return hanz.reply(m.chat, 'Kamu tidak memiliki cukup money untuk membuat guild. Butuh 20.000.000.000 money.', m);
    }
    ...
    if (user.guild) return hanz.reply(m.chat, 'Kamu sudah tergabung dalam guild.', m);
    ...
    if (!global.db.data.guilds[guildId]) {
        ...
        hanz.reply(m.chat, `Guild ${guildName} berhasil dibuat.`, m);
    } else {
        hanz.reply(m.chat, 'Terjadi kesalahan saat membuat guild. Coba lagi.', m);
    }
};
