...
try {
    if (/petshop/i.test(command)) {
      const count = args[1] && args[1].length > 0 ? Math.min(99999999, Math.max(parseInt(args[1]), 1)) : !args[1] || args.length < 3 ? 1 : Math.min(1, count)
        switch (type) {
          ...
          if (user.cat > 0) return m.reply('Kamu Sudah Memiliki Pet Ini!')
            ...
            hanz.reply(m.chat, `Selamat Kamu Mendapatkan Pet Cat`, m)
            break
          case 'dog':
          if (user.dog > 0) return m.reply('Kamu Sudah Memiliki Pet Ini!')
            ...
            hanz.reply(m.chat, `Selamat Kamu Mendapatkan Pet Dog`, m)
            break
          case 'fox':
          if (user.fox > 0) return m.reply('Kamu Sudah Memiliki Pet Ini!')
            ...
            hanz.reply(m.chat, `Selamat Kamu Mendapatkan Pet Fox`, m)
            break
          case 'horse':
          if (user.horse > 0) return m.reply('Kamu Sudah Memiliki Pet Ini!')
            ...
            hanz.reply(m.chat, `Selamat Kamu Mendapatkan Pet Horse`, m)
            break
          case 'petfood':
          if (global.db.data.users[m.sender].money >= hpetfood * count) {
            ...
            hanz.reply(m.chat, `Berhasil Membeli *${count}* Pet Food, Sebesar *Rp.${hpetfood * count}*`, m)
          } else hanz.reply(m.chat, `Uang Kamu Tidak Mencukupi`, m) 
            break
      ...
      switch (_type) {
        ...
        default:
          return hanz.reply(m.chat, caption, m)
      }
    }
  } catch (err) {
    m.reply("Error\n\n\n" + err.stack)
