let handler = async (m, { command, usedPrefix, args,hanz }) => {
    ...
    switch (type) {
      case "wine":
        if (user.stamina < 100) {
          ...
          } else hanz.reply(m.chat, ` wine kamu kurang`, m);
      ...
      case "potion":
        if (user.health < 100) {
          ...
          } else hanz.reply(m.chat, `kamu Tidak Memiliki Potion Yang Cukup`, m);
        } else hanz.reply(m.chat, `Darah Mu Sudah Penuh`, m);
        break;
      case "beer":
        if (user.level < 200) {
          ...
          } else hanz.reply(m.chat, `Beer Mu Tidak Cukup`, m);
        } else
          hanz.reply(
            m.chat,
