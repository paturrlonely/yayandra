...
let handler = async (m, { hanz }) => {
    ...
    if (!guildId) {
        return hanz.reply(m.chat, 'Anda harus bergabung dengan sebuah guild untuk menggunakan perintah ini.', m);
    }

    let guild = global.db.data.guilds[guildId];
    if (!guild) {
        return hanz.reply(m.chat, 'Guild Anda tidak ditemukan di basis data.', m);
    }

    if (guild.owner !== m.sender && !guild.staff.includes(m.sender)) {
        return hanz.reply(m.chat, 'Anda tidak memiliki izin untuk menyerang guild lawan.', m);
    }

    hanz.reply(m.chat, 'Mencari Guild Aktif 🔎', m);
    ...
    setTimeout(async () => {
        ...
        if (!attackedGuild) {
            return hanz.reply(m.chat, 'Tidak ada guild lawan yang dapat diserang saat ini.', m);
        }

        hanz.reply(m.chat, `Menemukan Guild Aktif ${attackedGuild.name}`, m);
        ...
        hanz.reply(m.chat, `Memulai Penyerangan Menggunakan ${itemName}`, m);

        await sleep(getRandomInt(1000, 5000)); // Jeda 1-5 detik

        hanz.reply(m.chat, `${guild.name} VS ${attackedGuild.name}`, m);
