let confirmation = {};
async function handler(m, { hanz, args, usedPrefix, command }) {
  ...
  let { key } = await hanz.reply(m.chat, confirm, m, {
    mentions: [m.sender],
  });
  confirmation[m.sender] = {
    ...
    timeout: setTimeout(
      () => (
        hanz.sendMessage(m.chat, { delete: key }), delete confirmation[m.sender]
      ),
      60 * 1000
    ),
  };
