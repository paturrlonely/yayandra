let handler = async (m, { hanz, text }) => {
  ...
  if (timeDiff < 600000) {
    ...
    hanz.reply(m.chat, `Anda harus menunggu ${remainingTime / 1000} detik lagi sebelum berkencan lagi. ⏳`, m);
    return;
  }
...
📍 Tempat Kencan: ${dateLocation}
      ...
      setTimeout(() => {
        ...
        hanz.reply(m.chat, `Kencan selesai!\n\n${dateInfo}\n\n${ending}`, m);
      }, 30000);

      hanz.reply(m.chat, `Anda sedang berkencan dengan ${partnerName}!\n\n${dateInfo}`, m);

      user.lastdate = currentTime;
    } else {
      hanz.reply(m.chat, 'Pilihan karakter tidak valid. Berkencan dibatalkan.', m);
    }
  } else {
    const characterList = getCharacterOptions().map((char, index) => `${index + 1}. ${char}`).join('\n');
    hanz.reply(m.chat, `Silakan pilih karakter dengan format .kencan [nomor karakter].\n\nList Karakter:\n${characterList}`, m);
  }
};
