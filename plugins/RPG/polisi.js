...
const handler = async (m, { hanz, usedPrefix, command, args }) => {
  const sender = m.sender;
  const user = global.db.data.users[sender]
  hanz.players = hanz.players || {};
  const player = hanz.players[sender] || { Uang: 0, Pencuri_Tertangkap: 0, Waktu_Tertangkap: 0, Kaca_Pembesar: 0, Level: 1, State: STATES.IDLE };

  if (command === "polisi") {
    if (args.length === 0) {
      hanz.reply(m.chat, "*👮‍♂️ Cara Bermain Game Polisi dan Pencuri 👮‍♂️*\n\n" +
    ...
    if (subCommand === "cari") {
      if (player.State !== STATES.IDLE) {
        return hanz.reply(m.chat, "*🔍 Sedang dalam pencarian...*", m);
      }

      if (Date.now() - player.Waktu_Tertangkap < 30000) {
        return hanz.reply(m.chat, "*⏱️ Anda harus menunggu sebentar sebelum dapat mencari kembali.*", m);
      }
      ...
      if (args.length === 1) {
        ...
        if (item === "kaca-pembesar") {
          if (player.Kaca_Pembesar) {
            return hanz.reply(m.chat, "*🛒 Anda sudah memiliki kaca pembesar.*", m);
          }

          if (player.Uang < 200) {
            return hanz.reply(m.chat, "*🛒 Uang Anda tidak cukup untuk membeli kaca pembesar.*", m);
          }

          player.Kaca_Pembesar = 1;
          player.Uang -= 200;
          hanz.reply(m.chat, "*🛒 Anda berhasil membeli kaca pembesar.* Gunakan 'polisi cari' untuk meningkatkan peluang menangkap pencuri.", m);
        } else {
          hanz.reply(m.chat, "*🛒 Item yang dimaksud tidak ditemukan.*", m);
        }
      }
    } else if (subCommand === "leaderboard") {
      // Sort players based on the number of thieves caught (descending order)
      const leaderboard = Object.entries(hanz.players)
      ...
      hanz.reply(m.chat, leaderboardMsg, m);
    ...
    user.money += player.Uang * player.Pencuri_Tertangkap;
      ...
      hanz.reply(m.chat, `*👮‍♂️ Sesi permainan Polisi dan Pencuri telah dihentikan.*\n\n${skorMsg}`, m);
      player.State = STATES.IDLE;
      player.ThiefAction = undefined;
    } else {
      if (player.State !== STATES.SEARCHING) {
        return hanz.reply(m.chat, "*🔍 Anda harus mencari pencuri terlebih dahulu dengan perintah 'polisi cari'.*", m);
      }
      ...
      if (!thiefActions[level].includes(polisiAction)) {
        return hanz.reply(m.chat, `*🚓 Pilihan tindakan Anda (${polisiAction.toUpperCase()}) tidak sesuai dengan hasil yang dicari.*`, m);
      }
      ...
      if (thiefActions[level].includes(player.ThiefAction)) {
        ...
        hanz.reply(m.chat, `*🚓 Anda berhasil melawan dan menangkap pencuri level ${level}!* Anda mendapatkan imbalan Rp${reward.toLocaleString()}. Total Uang Anda: Rp${player.Uang.toLocaleString()}.`, m);
      } else {
        hanz.reply(m.chat, "*🚓 Tindakan Anda tidak tepat dan pencuri berhasil lepas!*", m);
      }
    ...
    hanz.players[sender] = player;
  } else if (command === "info") {
    hanz.reply(m.chat, "*ℹ️ Gunakan perintah 'polisi' untuk memulai game Polisi dan Pencuri.*", m);
  }
};
