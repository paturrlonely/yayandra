...
var { id, subject,creation,desc,descId,participants,owner,subjectOwner } = await hanz.groupGetInviteInfo(kode).catch(async () => {
...
thumbnailUrl: 'https://pomf2.lain.la/f/762x0amm.jpg'
}
}
...
try{
 var nana = await hanz.groupAcceptInvite(kode) 
} catch{
  var nana = undefined
}
...
let groupMetadata = nana == undefined? {} : await hanz.groupMetadata(id) 
...
• Days: ${hanz.msToDate(toMs(Second))}
...
await hanz.sendMessage(m.chat,{text:teks,mentions:[tagnya]},{quoted:m})
if(nana == undefined ) return m.reply('Waiting for approval to join group')


await sleep(2000)
await hanz.sendMessage(id,{text:teks,contextInfo})
let ini = `Halo kak aku ${fake} silakan ketik .menu`
//await hanz.sendMessage(id,{text:ini,mentions:[tagnya]})
...
function add(gid, subject, link, expired) {
  ...
  chat.creator = creator && creator.includes('@') 
    ? 'wa.me/' + creator.split('@')[1].replace(new RegExp("[()+-/ +/]", "gi"), "") 
    : 'Tidak ada';
  chat.name = subject;
} else {
  global.db.data.chats[gid] = {
    ...
    creator: creator && creator.includes('@') 
      ? 'wa.me/' + creator.split('@')[1].replace(new RegExp("[()+-/ +/]", "gi"), "") 
      : 'Tidak ada',
    name: subject
  };
