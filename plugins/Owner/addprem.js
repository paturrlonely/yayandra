...
let handler = async (m, { hanz, usedPrefix, command,isOwner,q,setReply }) => {
...
let {id, subject,creator,creation,desc,descId} = await hanz.groupGetInviteInfo(code).catch(async () => {return setReply("Invalid invite url.")});
const metaData = await hanz.groupMetadata(id)
...
let namanye = await hanz.getName(i.id)
