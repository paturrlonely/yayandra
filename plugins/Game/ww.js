async function handler(m, { hanz, usedPrefix, command, text,args }) {
    ...
    if (value === "create") {
        ...
        if (ww[chat].cooldown > 0) {
            if (ww[chat].time === "voting") {
                ...
                addTimer(chat, ww);
                return await run_vote(hanz, chat, ww);
            } else if (ww[chat].time === "malem") {
                clearAllVote(chat, ww);
                addTimer(chat, ww);
                return await run_malam(hanz, chat, ww);
            } else if (ww[chat].time === "pagi") {
                clearAllVote(chat, ww);
                addTimer(chat, ww);
                return await run_pagi(hanz, chat, ww);
            }
        }
        if (ww[chat].status === true)
        ...
        for (let i = 0; i < ww[chat].player.length; i++) {
            ...
            if (ww[chat].player[i].role === "werewolf") {
              ...
              "werewolf"
                    ...
                    await hanz.sendMessage(ww[chat].player[i].id, listMessage);
                   
                    await hanz.sendMessage(ww[chat].player[i].id, {
                        text: textt,
                        mentions: player,
                    });
                }
...
handler.command = handler.help = ["werewolf", "ww"];
handler.tags = ["game"];
