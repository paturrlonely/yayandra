let handler = async (m, { hanz,setReply, args, command }) => {
  try {
    ...
    if (!args[0] || isNaN(args[0])) {
      return conn.reply(m.chat, `Masukkan jumlah balance yang ingin ditransfer.\nContoh: .${command} 100`, m);
    }
    ...
    if (global.db.data.users[sender].balance < amount) {
      return setReply(`Saldo kamu tidak mencukupi untuk transfer sebesar Rp.${amount.toLocaleString()}.`);
    }
    ...
    // Proses transfer
    global.db.data.users[sender].balance -= amount;
    global.db.data.users[receiver].balance += amount;

    // Balasan sukses
    return hanz.reply(
      m.chat,
      `✅ Berhasil mentransfer balance ebesar Rp.${amount.toLocaleString()} ke nomor ${receiver.split("@")[0]}.`,
      m
    );
  } catch (err) {
    console.error(err);
