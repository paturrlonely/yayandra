...
const handler = async (m, { conn, usedPrefix, text, participants, groupMetadata }) => {
    if (!text) {
        ...
        */
        let caption = `*Pesan Cinta...* ${toM(a)} ❤️ ${toM(b)}\n${await ktnmbk[Math.floor(Math.random() * ktnmbk.length)]}`;
    ...
    try {
        ...
        if (!users) return conn.reply(m.chat, `*_Doi Atau Nomor Tidak Ditemukan, Mungkin Sudah Keluar Atau Bukan Anggota Grup Ini Dan Tidak Ada Di Database Bot Silakan Daftar Terlebih Dahulu Ketik .daftar*_`, m);
