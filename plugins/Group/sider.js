const handler = async (m, { hanz, text, groupMetadata }) => {
    await hanz.sendPresenceUpdate('composing', m.chat);
    var lama = 86400000 * 7; // 7 hari dalam milidetik
    ...
    if (total == 0) return hanz.reply(m.chat, `*Digrup ini tidak terdapat sider.*`, m);
    hanz.reply(m.chat, `*${total}/${sum}* anggota grup *${await hanz.getName(m.chat)}* adalah sider dengan alasan:\n1. Tidak aktif selama lebih dari 7 hari\n2. Baru join tetapi tidak pernah nimbrung\n\n_“${pesan}”_\n\n*LIST SIDER:*\n${sider.map(v => '  ○ @' + v.replace(/@.+/, '' + typeof global.db.data.users[v] == "undefined" ? ' Sider ' : ' Off ' + msToDate(milliseconds - global.db.data.users[v].lastseen))).join('\n')}`, m, {
        contextInfo: {
            mentionedJid: sider
        }
    });
