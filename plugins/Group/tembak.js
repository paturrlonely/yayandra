...
let handler = async (m, { hanz, usedPrefix, text, participants, groupMetadata }) => {
    if (!text) {
    ...
    if (!text && !m.quoted) return hanz.reply(m.chat, `Masukan Nomor Atau Tag Doi, Bisa Juga Balas Pesan Doi`, m);

    if (isNaN(number)) return hanz.reply(m.chat, `_*Nomor Tidak Valid.*_`, m);
    if (number.length > 15) return hanz.reply(m.chat, `*_Format Tidak Valid.*_`, m);
    try {
        if (text) {
        ...
        if (!users) return hanz.reply(m.chat, `*_Doi Atau Nomor Tidak Ditemukan, Mungkin Sudah Keluar Atau Bukan Anggota Grup Ini.*_`, m);
        if (user === m.sender) return hanz.reply(m.chat, `_*Tidak Bisa Berpacaran Dengan Diri Sendiri.*_`, m);
        if (user === hanz.user.jid) return hanz.reply(m.chat, `_*Maaf Kak, Tapi Rangel Sepenuh Nya Milik Ehanz. :')*_`, m);
