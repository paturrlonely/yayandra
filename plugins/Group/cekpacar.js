let handler = async (m, { hanz, usedPrefix, text }) => {
  ...
  if(number.length > 15 || (number.length < 9 && number.length > 0)) return hanz.reply(m.chat, `Maaf, Nomor Yang Kamu Masukan Salah!`, m)
  ...
  if (global.db.data.users[user].pasangan == "") {
    hanz.reply(m.chat, `${orang} Tidak Memiliki Pasangan Dan Tidak Sedang Menembak Siapapun\n\n*Ketik .tembak @user Untuk Menembak Seseorang*`, m)
  }else if (global.db.data.users[global.db.data.users[user].pasangan].pasangan != user){
    hanz.reply(m.chat, `${orang} Sedang Menunggu Jawaban Tidak Pasti Dari @${global.db.data.users[user].pasangan.split('@')[0]} Karena Sedang Tidak Diterima Atau Di Tolak\n\nKetik *${usedPrefix}ikhlasin* Untuk Mengikhlaskan!`, m,{contextInfo: {
      mentionedJid: [global.db.data.users[user].pasangan]
    }})
  }else {
    hanz.reply(m.chat, `${orang} Sedang Menjalani Hubungan Dengan @${global.db.data.users[user].pasangan.split('@')[0]} 💓💓💓`, m,{contextInfo: {
      mentionedJid: [global.db.data.users[user].pasangan]
    }})
  }
}
