...
let handler = async (m, { prefix,hanz, command, q,setReply,isOwner}) => {
...
let { id, subject,creation,desc,descId,participants,owner,subjectOwner } = await hanz.groupGetInviteInfo(kode).catch(async () => {
...
await hanz.sendMessage(m.chat,{text,contextInfo,mentions:[tagnya]})
if(nana == undefined ) return m.reply('Waiting for approval to join group')
await sleep(2000)
await hanz.sendMessage(id,{text,contextInfo,mentions:[tagnya]})
await  hanz.groupLeave(idGc)
