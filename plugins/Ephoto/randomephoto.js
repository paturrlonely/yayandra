...
let handler = async (m, { hanz,usedPrefix, setReply, command, q }) => {
  try {
    if (!q) return setReply(`Example: ${usedPrefix + command} angel Dan Ehanz`);
setReply(mess.wait)
    // Menentukan link berdasarkan perintah
    let link;
    ...
    // Mengirim gambar
    await hanz.sendMessage(m.chat, { image: { url: haldwhd }, caption: 'Proses selesai, berikut hasilnya!' }, { quoted: m });
